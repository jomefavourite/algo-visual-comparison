...
export default class TimSort extends Algorithm {
   ...
   */
  static DEFAULT_MIN_MERGE = 32;

  /**
   * Minimum ordered subsequece required to do galloping.
   */
  static DEFAULT_MIN_GALLOPING = 7;

  /**
   * Default tmp storage length. Can increase depending on the size of the
   * smallest run to merge.
   */
  static DEFAULT_TMP_STORAGE_LENGTH = 256;

  /**
   * Pre-computed powers of 10 for efficient lexicographic comparison of
   * small integers.
   */
  static POWERS_OF_TEN = [1, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9];
   ...
   */
  makeAscendingRun(array, lo, hi, compare) {
    let runHi = lo + 1;
   ...
   * @param {function} compare - Item comparison function.
   */
  binaryInsertionSort(array, lo, hi, start, compare) {
    if (start === lo) {
      start++;
    }
   ...
   */
  gallopLeft(value, array, start, length, hint, compare) {
    let lastOffset = 0;
    let maxOffset = 0;
    let offset = 1;
   ...
   * @return {number} - The index where to insert value.
   */
  gallopRight(value, array, start, length, hint, compare) {
    let lastOffset = 0;
    let maxOffset = 0;
    let offset = 1;
  ...
  static TimSortInner = class {
    ...
    constructor(array, compare, outer) {
      this.array = array;
    ...
    mergeAt(i) {
       ...
       */
      const k = this.outer.gallopRight(
        array[start2],
        array,
        start1,
        length1,
    ...
    mergeLow(start1, length1, start2, length2) {
      ...
      while (true) {
        ...
        do {
          ...
          count1 >= TimSort.DEFAULT_MIN_GALLOPING ||
          count2 >= TimSort.DEFAULT_MIN_GALLOPING
        );
  ...
  sort(
    array,
    compare = this.alphabeticalCompare.bind(this),
    lo = 0,
    hi = array.length
  ) {
    let remaining = hi - lo;
